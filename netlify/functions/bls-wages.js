...
function hasUsableData(wages) {
  // Returns true if at least one SOC code has at least one wage level
...
export const handler = async (event) => {
  ...
  const { socCodes = [], stateFips = '00', msaCode = '' } = body
  ...
  const emptyResponse = (warning) => ({
    ...
    body: JSON.stringify({ wages: {}, geoLevel: 'none', warning }),
  })
  ...
  if (msaCode) {
    ...
    if (msaWages && hasUsableData(msaWages)) {
      return {
        statusCode: 200,
        headers: { 'Content-Type': 'application/json', 'Access-Control-Allow-Origin': '*' },
        body: JSON.stringify({ wages: msaWages, geoLevel: 'msa' }),
      }
    }
  ...
  return {
    statusCode: 200,
    headers: { 'Content-Type': 'application/json', 'Access-Control-Allow-Origin': '*' },
    body: JSON.stringify({ wages: stateWages, geoLevel: 'state' }),
  }
}
